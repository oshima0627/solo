  ...
  return (
    <Screen>
      ...
      <div>
        ...
        <p
          className={`mt-2 font-serif text-6xl leading-none ${
            isBomb ? 'animate-strike text-vermilion' : 'animate-rise'
          }`}
        >
          {info.callout}
        </p>
        {info.note ? <p className="mt-4 text-xs text-ink-faint">{info.note}</p> : null}
        {isBomb ? (
        ...
        {result.deckSwapped ? (
          <p className="animate-rise mt-4 border-t border-rule pt-3 text-sm">
      ...
      <div className="flex-1 overflow-y-auto">
        {game.config.players.map((player) => {
          ...
          return (
            <div
              key={player.id}
              className="flex items-center gap-4 border-b border-rule py-3.5 first:border-t"
            >
